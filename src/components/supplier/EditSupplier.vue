<template>
  <Card>
    <template #content>
      ...
      >
        <b class="text-xl">Edit Supplier</b>
      </Divider>
...
import { createRecordApi, getRecordApi, updateRecordApi } from '@src/api/endpoints';
...
const findSupplier = async () => {
  const res: any = await getRecordApi(`/suppliers/${route.params.supplier_id}`);
  formValues.value = res?.data;
  if (res.data?.address) {
    res.data?.address.map((item: any) => {
      if (item.type === 'shipping') {
        console.log(item);
        formValues.value.shipping_address = item;
      } else {
        formValues.value.mailing_address = item;
      }
    });
  }
};
...
const handleSubmit = async () => {
  ...
  if (formValues?.value.name !== undefined) {
    const res: any = await updateRecordApi(`/suppliers/${formValues.value.id}`, formValues.value);
