<template>
  <Card>
    <template #content>
      ...
      >
        <b class="text-xl">Edit Product</b>
      </Divider>
      <Form
        v-slot="$form: any"
        :resolver="resolver"
...
const findProduct = async () => {
  const res: any = await getRecordApi(`/products/${route.params.product_id}`);
...
const onFormSubmit = async ({ valid, values }: FormSubmitEvent) => {
  if (valid) {
    const res: any = await updateRecordApi(`/products/${route.params.product_id}`, values);
